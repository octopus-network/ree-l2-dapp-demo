import { useEffect, useState } from "react"
import Dexie from 'dexie';
import { getInscriptionImg } from "api/unisat";
import { Image } from "antd";

const fallbackImage = "data:image/png;base64,"

// define DB
interface ImageDB {
  id: string;
  blob: Blob;
  timestamp: number;
}

class ImageDatabase extends Dexie {
  images!: Dexie.Table<ImageDB, string>;

  constructor() {
    super('ImageDatabase');
    this.version(1).stores({
      images: 'id, timestamp'
    });
  }
}

const db = new ImageDatabase();


export function InscriptionImg({
  inscriptionId,
  className
}: {
  inscriptionId: string,
  className?: string | undefined
}) {

  const [errorMsg, setErrorMsg] = useState<string | undefined>(undefined)
  const [imgSrc, setImgSrc] = useState<string | undefined>(undefined);
  const [isLoading, setIsLoading] = useState<boolean>(true)


  useEffect(() => {
    if (inscriptionId === undefined) {
      setErrorMsg('InscriptionId is undefined')
      setIsLoading(false)
      return
    }

    const loadImg = async () => {
      let image = await db.images.get(inscriptionId)

      // if get image from db
      if (image) {
        setImgSrc(URL.createObjectURL(image.blob))
        setIsLoading(false)
        return
      }

      // if image not in db, fetch it

      const response = await fetch(`${import.meta.env.VITE_UNISAT_OPEN_API_URL}/v1/indexer/inscription/content/${inscriptionId}`);
      if (response.ok) {
        const blob = await response.blob();
        db.images.add({
          id: inscriptionId,
          blob: blob,
          timestamp: Date.now()
        })
        setImgSrc(URL.createObjectURL(blob))
        setIsLoading(false)
      } else {
        setErrorMsg(`Failed to fetch image: ${response.statusText}`);
        setIsLoading(false)
        return
      }
    }

    loadImg()


  }, [inscriptionId])

  return (
    <img
      src={imgSrc || fallbackImage}
      className={className}
      alt={errorMsg}
    />
  )
}